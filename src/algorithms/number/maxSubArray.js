...
export default class maxSubArray extends Algorithm {
    ...
    divideAndConquer = (array) => {
        let { high, low, sum } = this.findMaximumSubArray(array, array.length - 1, 0);
    ...
    findMaximumSubArray = (array, high, low) => {
        ...
        let { low: leftLow, high: leftHigh, sum: leftSum } = this.findMaximumSubArray(array, mid, low);
        let { low: rightLow, high: rightHigh, sum: rightSum } = this.findMaximumSubArray(array, high, mid + 1);

        let { maxLeft: crossLow, maxRight: crossHigh, sum: crossSum } = this.findMaxCrossingSubArray(array, low, mid, high);
        let diffLeft = leftHigh - leftLow;
        let diffRight = rightHigh - rightLow;
        let diffCross = crossHigh - crossLow;

        if (leftSum >= rightSum && leftSum >= crossSum)
            if (!(leftSum == rightSum && (diffLeft > diffRight || diffLeft > diffCross)))
